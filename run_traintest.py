...
def main(hparam, checkpoint_dir=None, run_hparam=False, data_limit=False):
  ...
  # Define directories
  # ---------------------------------------------------------------------------------
  TARGET_FOLDER_PATH = os.path.join(deeprad_dir, "data", "traintest3", "out_data")
  CHANNEL_FOLDER_PATH = os.path.join(deeprad_dir, "data", "traintest3", "in_data")
  ...
  for epoch in range(max_epochs):
    ...
    for i, (input_batch, label_batch) in enumerate(test_loader):
        ...
        test_loss = test_loss + loss.cpu().item() / batch_size
...
if __name__ == "__main__":
  ...
  if len(sys.argv) > 1:
    ...
    if '--data_limit' in argv:
      i = argv.index('--data_limit')
      arg_dict['data_limit'] = int(argv[i + 1])
  ...
  if not run_hparam:
    ...
    hparam['f1'] = 36 # 24
    hparam['k1'] = 3
    main(hparam, None, run_hparam=run_hparam, data_limit=data_limit)
